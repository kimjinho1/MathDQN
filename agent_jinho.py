...
class Agent:
    ...
    def compound_two_nodes_predict(self, op, loc, prefix="./test/analysis_"):
        ...
        if len(self.state.nodes) == 1:
            #print('fix_node1: ', fix_node1.value)
            #print('fix_node2: ', fix_node2.value)
            #print('self.state.nodes[0].value: ', self.state.nodes[0].value)
            #print('self.gold_tree.gold_ans: ', self.gold_tree.gold_ans) 
            if abs(float(self.state.nodes[0].value) - float(self.gold_tree.gold_ans)) < 1e-4:
                ...
                return np.zeros(self.feat_dim), 1, 1, fix_node1.value, fix_node2.value, self.state.nodes[0].value, self.gold_tree.gold_ans, time.time() - start 
            else:
                #self.write_single_info(filename, 1, "compute_state_node==1", "_error")
                return np.zeros(self.feat_dim), 1, 0, fix_node1.value, fix_node2.value, self.state.nodes[0].value, self.gold_tree.gold_ans, time.time() - start 
            ...
            return next_states, 0, 0, fix_node1.value, fix_node2.value, self.state.nodes[0].value, self.gold_tree.gold_ans, time.time() - start 
